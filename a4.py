...
class MainApp(tk.Frame):
    ...
    def _initialize_messenger(self):
        if self.username and self.password and self.server:
            ...
            print(f"Messenger initialized with user: {self.username}, server: {self.server}") #debugging

    def send_message(self):
        message = self.body.get_text_entry()
        if self.recipient and message:
            print(f"Sending message to {self.recipient}: {message}") #debugging
            ...
            print("Message sent and profile saved.")#debugging

    def add_contact(self):
        new_contact = simpledialog.askstring("Add Contact", "Enter the username of the new contact:")
        if new_contact:
            print(f"Adding new contact: {new_contact}") #debugging
            self.profile.add_friend(new_contact)
            self.body.insert_contact(new_contact)
            self.profile.save_profile("profile.dsu")
            print("Contact added and profile saved.") #debugging

    def recipient_selected(self, recipient):
        self.recipient = recipient
        print(f"Recipient selected: {recipient}") #debugging
    ...
    def configure_server(self):
        ...
        if self.username and self.password and self.server:
            ...
            self.profile.save_profile("profile.dsu")
            print(f"Server configured with user: {self.username}, server: {self.server}") #debugging
    ...
    def open_profile(self):
        ...
        if file_path:
            try:
                ...
                print(f"Profile loaded from {file_path}") #debugging
            except Exception as e:
                messagebox.showerror("Error", f"Failed to load profile: {e}") #debugging
    ...
    def save_profile(self):
        ...
        if file_path:
            try:
                ...
                print(f"Profile saved to {file_path}") #debugging
            except Exception as e:
                messagebox.showerror("Error", f"Failed to save profile: {e}") #debugging
    ...
    def check_new(self):
        if self.direct_messenger:
            print("Checking for new messages...") #debugging
            try:
                ...
                for msg in new_messages:
                    if isinstance(msg, ds_messenger.DirectMessage):
                        ...
                        print(f"New message from {new_message.sender}: {new_message.message}") #debugging
                        ...
                        print("Unexpected message format: ", msg) #debugging
                self.profile.save_profile("profile.dsu")
            except Exception:
